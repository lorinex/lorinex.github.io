...
全局替换是从所有帧的集合中选择一个替换帧，一个进程可以从另一个进程中取走一个帧。这是最容易实现的，也是常用的方法。操作系统维护一个空闲帧列表。当发生页面错误时，空闲帧被添加到该进程的常驻集中。问题之一是进程无法控制自己的页面错误率。局部替换则从遭受错误的进程的常驻集中选择页面。一个进程的内存中页面集合只受该进程的分页行为影响。全局替换结果是更高的系统吞吐量。
...
- Δ 代表工作集窗口大小，是一个固定的页面引用数量。例如10,000条指令。
- 工作集(Working Set)是最近Δ个页面引用中的页面集合。
- WSSi(Working Set Size of Process Pi)是进程Pi的工作集大小，也就是进程i需要的页面数量，它会随着时间变化。 
- 工作集的准确性取决于Δ的选择。如果Δ太小，工作集大小会偏小；如果Δ太大，工作集大小会偏大；如果Δ等于最大值，则工作集大小为最大值。

- D是所有进程的工作集大小之和，用来近似表示程序的局部性。如果D大于可用帧数m，就会发生抖动。操作系统需要监控每个进程的工作集，如果D大于m，就需要挂起一个进程。难点在于持续跟踪工作集。
-  跟踪工作集的方法是：
  - 使用定时器定期中断，例如每隔5000时间单位中断一次。
  - 为每个页面维护两个内存位，一个用于记录页面最近一次被访问的时间，一个用于记录页面是否在工作集中。
  - 在定时器中断时，将所有页面的访问时间位清零，并将工作集中的页面的访问时间位置1。
  - 根据页面访问时间位判断页面是否在工作集中。
  这种方法并不完全准确，因为页面的工作集可能会快速变化，定时器中断频率无法实时反映页面的工作集状态。
  改进方法是增加历史位数量，并提高定时器中断频率，例如使用10个历史位，每隔1000时间单位中断一次。这可以更准确地跟踪页面的工作集。
