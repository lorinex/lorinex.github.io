...
CREATE TRIGGER `insert_update_duplicate`
BEFORE INSERT ON `your_table` -- 请替换 `your_table` 为你的表名
FOR EACH ROW
BEGIN
  DECLARE existing_key INT;
  
  -- 检查新插入的数据的主键是否已存在
  SELECT `S_SUPPKEY` INTO existing_key FROM `your_table` WHERE `S_SUPPKEY` = NEW.`S_SUPPKEY`;
  
  IF existing_key IS NOT NULL THEN
    -- 如果主键已存在，根据新插入的数据更新已有元组的属性值
    UPDATE `your_table`
    SET `S_NAME` = NEW.`S_NAME`,
        `S_ADDRESS` = NEW.`S_ADDRESS`,
        `S_NATIONKEY` = NEW.`S_NATIONKEY`,
        `S_PHONE` = NEW.`S_PHONE`,
        `S_ACCTBAL` = NEW.`S_ACCTBAL`,
        `S_COMMENT` = NEW.`S_COMMENT`
    WHERE `S_SUPPKEY` = NEW.`S_SUPPKEY`;
  END IF;
END;
//
...
查看表 suppliercopy1 上的触发器，

```sql
SHOW TRIGGERS LIKE 'suppliercopy1';
```
...
VALUES (2001, 'Supplier#000002001', 'Address 2001', 14, '24-123-456-7890', 2500.00, 'New supplier with key 2001');
```
...
```sql
INSERT INTO suppliercopy1 (S_SUPPKEY, S_NAME, S_ADDRESS, S_NATIONKEY, S_PHONE, S_ACCTBAL, S_COMMENT)
VALUES (2001, 'Supplier#000002001', 'Address 2001', 14, '24-123-456-7890', 3500.00, 'New supplier with key 2001');
```
