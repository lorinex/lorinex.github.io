...
配置NFS服务器以允许访问共享目录。编辑/etc/exports 文件：

```
sudo nano /etc/exports
```

在文件末尾添加以下行（替换 <IP_or_subnet> 为允许访问共享目录的IP地址或子网。rw 表示可读可写权限）：

```
/srv/nfs/share <IP_or_subnet>(rw,sync,no_subtree_check)
```
...
挂载NFS共享到本地目录（替换 <server_IP> 为NFS服务器的IP地址，<share_directory> 为共享的目录路径）：
...
若要使NFS共享在启动时自动挂载，你可以编辑` /etc/fstab` 文件并添加以下行：
...
使⽤ root ⽤户登录到集群的 sys 租户。

```
obclient -h127.0.0.1 -P2881 -uroot@sys -p12345678 -A
```
...
![](https://picture2023-1309715649.cos.ap-beijing.myqcloud.com/img/20231219175309.png)

创建⼀个名称为 S1_unit_config 的资源规格，其资源配置为 CPU 为 1 核，内存 2G，⽇志盘空间6G。
...
![](https://picture2023-1309715649.cos.ap-beijing.myqcloud.com/img/20231219175420.png)
...
![](https://picture2023-1309715649.cos.ap-beijing.myqcloud.com/img/20231219180016.png)
...
![](https://picture2023-1309715649.cos.ap-beijing.myqcloud.com/img/20231219180147.png)
...
ALTER RESOURCE UNIT S1_unit_config MEMORY_SIZE = '2G';
